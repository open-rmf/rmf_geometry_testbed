  ...
  const sf::Color& color = sf::Color(255, 255, 255, 255))
{
  ...
  for (uint i=0; i<shapes.size(); ++i)
  {
    ...
    IMDraw::draw_circle(sf::Vector2f(pt.x(), pt.y()), 
    ...
    if (i == 0)
    {
      ...
      IMDraw::draw_arrow(sf::Vector2f(pt.x(), pt.y()), sf::Vector2f(pt_end.x(), pt_end.y()), color);
    }
  }
}
...

    {
      ...
      if (preset_changed)
      {
        if (current_preset != -1 && current_preset < (int)presets.size())
        {
          ...
          auto knots_a =
            compute_knots(a_start, a_end, zero, zero);
          auto knots_b =
            compute_knots(b_start, b_end, b_vel, -b_vel);
